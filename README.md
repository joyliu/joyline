# Joyline Design Mentorship

[![License: CC BY-SA 4.0](https://img.shields.io/badge/License-CC%20BY--SA%204.0-lightgrey.svg)](https://creativecommons.org/licenses/by-sa/4.0/)
![Knowledge Base: 100 Entries](https://img.shields.io/badge/KB%20Entries-100-blue)
![Status: Active](https://img.shields.io/badge/Status-Active-green)

15 years of design mentorship experience, distilled into 100 Q&A entries covering portfolios, career navigation, collaboration, leadership, and navigating the design industry.

No gatekeeping. No $200 coffee chats. Just frameworks, honest advice, and practical guidance.

---

## Three Ways to Use This

Pick the mode that works for you:

### 🌐 Browse Mode
**Read the knowledge base directly**

Perfect for: Skimming topics, searching specific questions, understanding the structure

→ [View Knowledge Base](https://joyliujoyliu.notion.site/308a1b409bd081ffbfbad2d32a825118?v=308a1b409bd0811da4d1000cc12c94e5)

---

### ⚡ Easy Mode (Poe Bot)
**Chat with AskJoyline - no setup required**

Perfect for: Quick questions, tactical advice, getting unstuck fast

- One-click access: **[poe.com/AskJoyline](https://poe.com/AskJoyline)**
- Free tier: ~28 messages/day
- Powered by Claude Haiku 4.5
- Good for: "How do I write a handoff doc?", "Should I specialize?", "Portfolio tips?"

---

### 🔧 Pro Mode (Claude Skill)
**Deep coaching in your own Claude Project**

Perfect for: Ongoing conversations, personal context, multiturn coaching

- Requires: [Claude Pro subscription](https://claude.ai/upgrade)
- Setup time: ~5 minutes
- Uses: Claude Sonnet/Opus (better quality, unlimited messages)
- Good for: Career transitions, stakeholder challenges, ongoing growth

---

## What's Inside

100 curated Q&A entries organized into themes:

- **Career Navigation** - Transitions, promotions, growth paths
- **Mindset & Growth** - Burnout, resilience, learning
- **Portfolio & Interviewing** - Case studies, presentations, interview prep
- **Collaboration & Influence** - Working with PMs/engineers, stakeholder management
- **Industry & Trends** - AI, tools, market shifts
- **Design Strategy & Vision** - Roadmaps, trade-offs, defending decisions
- **Org Design & Ops** - Team structure, reviews, succession planning

Each entry follows this structure:
- **Context** - Why people ask this question
- **Reframe** - What the real question is
- **Lesson hook** - The principle or mindset shift
- **Answer** - Frameworks, examples, level-specific guidance
- **Lesson** - Explicit takeaway you can apply

---

## Example Entry

**Q: Should I specialize or stay generalist?**

**Reframe:** The real question isn't "specialist or generalist?" — it's "How do I stay resilient in a shifting market?"

**Answer:** Right now, with tighter budgets and smaller teams, the most resilient designers are **generalists with spikes**: broad enough to handle end-to-end work, but deep enough to be recognized for specific strengths.

Here's the approach:
- **Be great at a few skills** — your superpowers
- **Be good at most core skills** — work efficiently across the design process
- **Be proficient in niche skills** — speak the language with engineers, PMs, researchers

The riskiest place to be? A specialist who needs constant coverage in other areas. If your manager has to pull extra people just to make you effective, your role becomes harder to justify.

**Lesson:** Don't choose between specialist and generalist. Build breadth so you can contribute across the spectrum, then go deep in a couple of areas so you stand out.


---

## Who This Is For

- **Junior designers** navigating their first few years
- **Career changers** transitioning into design
- **Mid-level designers** trying to level up
- **Senior designers** considering leadership
- **Anyone** wondering if UX is right for them

---

## Who This Isn't For

This is mentorship guidance, not:
- Therapy or medical advice
- Personal coaching for specific workplace situations
- Resume writing or portfolio reviews
- Promises of job outcomes

If you need deeper 1:1 support, consider:
- [Joy's coffee chats](https://www.joy-liu.com/coffee-chat)
- Building your own Claude coaching project
- Professional career coaching

---

## How to Use the Claude Skill (Pro Mode)

**5-minute setup:**

1. Sign up for [Claude Pro](https://claude.ai/upgrade) ($20/month)
2. Create a new Project in Claude
3. Copy the contents of [SKILL.md](SKILL.md) into the Project's custom instructions
4. Upload [knowledgebase.md](knowledgebase.md) as Project knowledge
5. Start asking questions!


---

## Contributing

Found a question that should be added? Spotted an issue?

- **Submit questions:** [Share via this form](https://joyliujoyliu.notion.site/308a1b409bd0819fb559d0a0229e9f93)
- **Report issues:** Open a GitHub issue

---

## License & Attribution

This work is licensed under [CC BY-SA 4.0](https://creativecommons.org/licenses/by-sa/4.0/).

**What this means:**
- ✅ Use it freely (personal, educational, commercial)
- ✅ Modify and adapt it for your needs
- ✅ Build tools and products with it
- ⚠️ You must credit Joy Liu and link back to this repo
- ⚠️ Derivative works must use the same license

**In plain English:** Don't be a jerk. If this helped you, say so. If you build on it, share back with the community. Don't copy and pretend you made this.

---

## Feedback

If this helped you land an interview, feel confident in a tough conversation, or navigate a career decision—let me know. That's the biggest reward.

- **Submit feedback:** [Via this form](https://joyliujoyliu.notion.site/308a1b409bd0819fb559d0a0229e9f93)
- **Drop a comment** on [LinkedIn](https://www.linkedin.com/in/enjoyinjoy/)
- **Star this repo** ⭐ if you found it useful

---

## Changelog

### v1.0.0 (2026/03/01)
- Initial release with 100 knowledge base entries
- Launched AskJoyline on Poe (Easy Mode)
- Published Claude Skill setup (Pro Mode)
- Open-sourced full knowledge base

---

**Made with ☕ and 15 years of design experience**
